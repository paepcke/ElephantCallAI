...
Being modified to create spectrogram, operate on it,
then create freq_gated.wav
==========
...
class FrequencyGater(object):
    ...
    def __init__(self,
        ...
        FrequencyGater.log = LoggingService(logfile=logfile)
        ...
        if not testing:
            try:
                ...
                print(f"Cannot read .wav file '{wav_file_path}': {repr(e)}")
                sys.exit(1)
        ...
        if outfile is not None and not testing:
            ...
            wavfile.write(outfile, self.framerate, freq_gated_samples)
        ...
        if PlotterTasks.has_task('gated_wave_excerpt'):
            ...
            start_indx = self.find_busy_array_section(freq_gated_samples)
            ...
            self.plotter.plot(np.arange(start_indx, end_indx),
                              freq_gated_samples[start_indx:end_indx],
                              title=f"Frequency-Gated {os.path.basename(wav_file_path)}",
                              xlabel='Sample Index', 
                              ylabel='Voltage'
                              )
    ...
    def frequency_gate(self, samples_raw, spectrogram_dest=None):
        '''
        ...
        self.log.info(f"Applying front end band pass filter ({self.FRONT_END_HIGH_PASS_FREQ} to {self.FRONT_END_LOW_PASS_FREQ}")
        samples_low_freqs_removed = self.butter_lowpass_filter(samples_raw, 
                                                               self.FRONT_END_HIGH_PASS_FREQ, 
                                                               self.DEFAULT_BUTTERWORTH_ORDER)
        self.log.info("Done applying front end low pass filter.")        

        samples_band_passed= self.butter_lowpass_filter(samples_low_freqs_removed, 
        ...
        (freq_labels, time_labels, complex_freq_time) = \
            self.make_spectrogram(samples_band_passed)
        ...
        self.log.info(f"Finding freqs above {self.spectrogram_freq_cap}...") 
        high_freq_indices = np.nonzero((freq_labels > self.spectrogram_freq_cap) \
                                       | (freq_labels == 0.0))
        self.log.info(f"Done finding freqs above {self.spectrogram_freq_cap}.")
        
        self.log.info(f"Removing freq rows > {self.spectrogram_freq_cap}...") 
        freq_time_pruned = np.delete(freq_time, high_freq_indices, axis=0)
        freq_time_nulled = np.copy(freq_time)
        (_num_rows, num_cols) = freq_time_nulled.shape
        null_energy = np.zeros(num_cols, dtype=float)
        freq_time_nulled[high_freq_indices] = null_energy
        self.log.info(f"Done removing freq rows > {self.spectrogram_freq_cap}.") 
        
        # self.log.info(f"Removing corresponding freq labels...")
        # freq_labels_filtered = np.delete(freq_labels, high_freq_indices)
        # self.log.info(f"Done removing corresponding freq labels.")
        # assert(freq_time_filtered.shape[0] == len(freq_labels_filtered))
        freq_time_filtered = freq_time_nulled
        self.log.info(f"Converting energies to log10...")
        freq_time_log = np.log10(freq_time_filtered)
        self.log.info(f"Done converting energies to log10.")

        self.log.info(f"Converting frequency labels to log10...")
        freq_labels_log = np.log10(freq_labels)
        self.log.info(f"Done converting frequency labels to log10.")        
        
        #freq_time_dup = np.copy(freq_time)
        #num_rows = freq_time_dup.shape[0]
        #********
        #********freq_time_cleaned[10:,] = 0
        #freq_time_cleaned = np.delete(freq_time_dup, slice(10,num_rows), 0)
        #freq_time_cleaned = freq_time_dup
        #***freq_time_cleaned = freq_time_dup
        self.save_spectrogram(freq_time_log,
                              '/tmp/spectrotest.npy', 
                              freq_labels_log,
                              time_labels)
        #**** TODO:
        #   - Why is freq labels neg in freq_labels_log?
        #   - How to fill vertical space?
        #   - Inverse not working
        Plotter(8000).plot_spectrogram(freq_labels_log, 
                                       time_labels, 
                                       freq_time_log, 
                                       time_intervals_to_cover=[],
                                       block=True
                                       )
        #self.log.info("Leaving intentionally")
        #sys.exit()
        #********
        
        # Turn back to time domain
        samples_gated = self.make_inverse_spectrogram(freq_time_filtered)

        if spectrogram_dest is not None:
            self.save_spectrogram(freq_time_log, spectrogram_dest, freq_labels_log, time_labels)

        return samples_gated
    ...
    def amplitude_gate(self, 
        ...
        if order is None:
            order = FrequencyGater.DEFAULT_BUTTERWORTH_ORDER
