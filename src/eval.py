...
def visual_full_recall(spectrogram, predictions, labels, chunk_size=256):
    """
    ...
    while True:
        ...
        if (call_length > chunk_size):
            ...
            visualize(spectrogram[window_start: window_end], predictions[window_start: window_end], labels[window_start: window_end])
...
def call_prec_recall(test, compare, threshold=0.1, is_truth=False, spectrogram=None):
    """
        ...
        1) is_truth = True: try to find a call in compare that overlaps (threshold xs) for
        each call in test

        2) is_truth = False: for a call in test, try to find a call in compare that it
        overlaps (threshold xs).
...
def process_ground_truth(label_path, in_seconds=False, samplerate=8000, NFFT=3208., hop=641.):
    """
...
def find_elephant_calls(binary_preds, min_length=15, in_seconds=False, samplerate=8000., NFFT=3208., hop=641.):
    """
...
def test_full_spectrograms(dataset, model, model_id, sliding_window=True, 
            ...
            in_seconds=False, use_gt=False, min_call_lengh=15,
            visualize=False):
    """
    ...
    for data in dataset:
        ...
        binary_preds, smoothed_predictions = get_binary_predictions(predictions, min_length=min_call_lengh,
                        threshold=pred_threshold, smooth=smooth)
        ...
        print ("Num predicted chunks", len(predicted_calls))
        if use_gt:
        ...
        if visualize:
            visual_full_recall(spectrogram, processed_preds, labels)       
