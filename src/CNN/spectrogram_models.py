...
class ModelSelector(object):
    ...
    RAND_SEED = 2642
    # Map from model index to model name
    # for models used
    MODELS = {
        'resnet18': 17
        }
    ...
    def model_from_modname(self, model_name):
    ...
    def get_model(self, model_selector):
        ...
        if type(model_selector) == int:
            idx = model_selector
        else:
            idx = self.MODELS[model_selector]
    
        if idx == 0:
            return Model0(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 1:
            return Model1(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 2:
            return Model2(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 3:
            return Model3(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 4:
            return Model4(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 5:
            return Model5(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 6:
            return Model6(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 7:
            return Model7(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 8:
            return Model8(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 9:
            return Model9(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 10:
            return Model10(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 11:
            return Model11(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 12:
            return Model12(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 13:
            return Model13(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 14:
            return Model14(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 15:
            return Model15(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 16:
            return Model16(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
        elif idx == 17:
            return Model17(Defaults.INPUT_SIZE, Defaults.OUTPUT_SIZE, Defaults.LOSS, Defaults.FOCAL_WEIGHT_INIT)
...
class Model0(nn.Module):
    def __init__(self, input_size, output_size, loss="CE", weight_init=0.01):
        ...
        self.hidden_state = nn.Parameter(torch.rand(1, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
        self.cell_state = nn.Parameter(torch.rand(1, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
...
class Model1(nn.Module):
    def __init__(self, input_size, output_size, loss="CE", weight_init=0.01):
        ...
        self.hidden_state = nn.Parameter(torch.rand(self.num_layers, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
        self.cell_state = nn.Parameter(torch.rand(self.num_layers, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
...
class Model3(nn.Module):
    def __init__(self, input_size, output_size, loss="CE", weight_init=0.01):
        ...
        self.hidden_state = nn.Parameter(torch.rand(2*self.num_layers, 1, self.hidden_size), requires_grad=True).to(Defaults.device) # allow for bi-direct
        self.cell_state = nn.Parameter(torch.rand(2*self.num_layers, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
...
class Model16(nn.Module):
    def __init__(self, input_size, output_size, loss="CE", weight_init=0.01):
        ...
        self.hidden_state = nn.Parameter(torch.rand(2 * self.num_layers, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
        self.cell_state = nn.Parameter(torch.rand(2 * self.num_layers, 1, self.hidden_size), requires_grad=True).to(Defaults.device)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        alpha = torch.tensor([1 - self.alpha, self.alpha]).to(Defaults.device)
        ...
        for epoch in range(num_epochs):
            for phase in ['train', 'valid']:
                ...
                for inputs, labels, _ in dataloders[phase]:
                    i += 1
                    if (i % 1000 == 0) and Defaults.VERBOSE:
                    ...
                    inputs, labels = Variable(inputs.to(Defaults.device)), Variable(labels.to(Defaults.device))
            ...
            if all([val_accuracy < best_valid_acc for val_accuracy in last_validation_accuracies]):
                print("Early stopping because last {} validation accuracies have been {} and less than best val accuracy {}".format(Defaults.TRAIN_STOP_ITERATIONS, last_validation_accuracies, best_valid_acc))
                break
...
def adversarial_discovery(dataloader, model, num_files_to_return=-1, threshold=0.5, min_length=0):
    ...
    for inputs, labels, data_files in dataloader:
        ...
        binary_preds = torch.where(predictions > threshold, torch.tensor(1.0).to(Defaults.device), torch.tensor(0.0).to(Defaults.device))
...
# def main():    
#     ## Build Dataset
#     # "/home/jgs8/ElephantCallAI/elephant_dataset/Train_nouab/Neg_Samples_x" + str(Defaults.NEG_SAMPLES) + "/"a
#     train_loader = get_loader("../elephant_dataset/Train/Neg_Samples_x" + str(Defaults.NEG_SAMPLES) + "/", Defaults.BATCH_SIZE, random_seed=Defaults.DATA_LOADER_SEED, norm=Defaults.NORM, scale=Defaults.SCALE)
#     #train_loader = get_loader("../elephant_dataset/Train/Full_24_hrs", Defaults.BATCH_SIZE, random_seed=Defaults.DATA_LOADER_SEED, norm=Defaults.NORM, scale=Defaults.SCALE)
#     # Quatro
#     #train_loader = get_loader("/home/data/elephants/processed_data/Train_nouab/Full_24_hrs/", Defaults.BATCH_SIZE, random_seed=Defaults.DATA_LOADER_SEED, norm=Defaults.NORM, scale=Defaults.SCALE)
#     # The validation loader should be the full 24hr trainind data use for adversarial discovery
#     #validation_loader 
#     test_loader = get_loader("../elephant_dataset/Test/Neg_Samples_x" + str(Defaults.NEG_SAMPLES) + "/", Defaults.BATCH_SIZE, random_seed=Defaults.DATA_LOADER_SEED, norm=Defaults.NORM, scale=Defaults.SCALE)
#     # Quatro
#     #test_loader = get_loader("/home/data/elephants/processed_data/Test_nouab/Neg_Samples_x" + str(Defaults.NEG_SAMPLES) + "/", Defaults.BATCH_SIZE, random_seed=Defaults.DATA_LOADER_SEED, norm=Defaults.NORM, scale=Defaults.SCALE)
#     
...
#         ## Data Visualization
#         # model = torch.load(Defaults.MODEL_SAVE_PATH + Defaults.DATASET + '_model_' + sys.argv[2] + ".pt", map_location=Defaults.device)
#         model = torch.load(sys.argv[2], map_location=Defaults.device)
#         print(model)
# 
...
#         model = torch.load(sys.argv[2], map_location=Defaults.device)
# 
#         adversarial_files = adversarial_discovery(test_loader, model, min_length=Defaults.ADVERSARIAL_THRESHOLD)
...
#         model_id = tokens[-2]
#         with open(model_id + "_threshold_" + str(Defaults.ADVERSARIAL_THRESHOLD) + ".txt", 'w') as f:
...
# 
#         save_path = Defaults.SAVE_PATH + Defaults.DATASET + '_model_' + str(model_id) + "_" + Defaults.NORM + "_Negx" + str(Defaults.NEG_SAMPLES) + "_Loss_" + Defaults.LOSS + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()))
# 
...
#         writer.add_scalar('batch_size', Defaults.BATCH_SIZE)
#         writer.add_scalar('weight_decay', Defaults.HYPERPARAMETERS[model_id]['l2_reg'])
# 
#         #criterion = torch.nn.BCEWithLogitsLoss()
#         # Try focal loss
#         if Defaults.LOSS.upper() == "CE":
#             criterion = torch.nn.BCEWithLogitsLoss()
#             print ("Using Binary Cross Entropy Loss")
#         elif Defaults.LOSS.upper() == "FOCAL":
#             criterion = FocalLoss(alpha=Defaults.FOCAL_ALPHA, gamma=Defaults.FOCAL_GAMMA)
#             print ("Using Focal Loss with parameters alpha: {}, gamma: {}, pi: {}".format(Defaults.FOCAL_ALPHA, Defaults.FOCAL_GAMMA, Defaults.FOCAL_WEIGHT_INIT))
#         elif Defaults.LOSS.upper() == "FOCAL_CHUNK":
#             weight_func = None
#             if Defaults.CHUNK_WEIGHTING.lower() == "avg":
#                 weight_func = avg_confidence_weighting
#             elif Defaults.CHUNK_WEIGHTING.lower() == "count":
#                 weight_func = incorrect_count_weighting
# 
#             criterion = ChunkFocalLoss(weight_func, alpha=Defaults.FOCAL_ALPHA, gamma=Defaults.FOCAL_GAMMA)
#             print ("Using Chunk Based Focal Loss with weighting function: {}, alpha: {}, gamma: {}, pi: {}".format(
#                     Defaults.CHUNK_WEIGHTING, Defaults.FOCAL_ALPHA, 
#                     Defaults.FOCAL_GAMMA, Defaults.FOCAL_WEIGHT_INIT))
#         else:
#             print("Unknown Loss")
#             return
# 
#         optimizer = torch.optim.Adam(model.parameters(), lr=Defaults.HYPERPARAMETERS[model_id]['lr'], weight_decay=Defaults.HYPERPARAMETERS[model_id]['l2_reg'])
#         scheduler = torch.optim.lr_scheduler.StepLR(optimizer, Defaults.HYPERPARAMETERS[model_id]['lr_decay_step'], gamma=Defaults.HYPERPARAMETERS[model_id]['lr_decay'])
# 
#         start_time = time.time()
#         model_wts = None
# 
#         model_wts = train_model(dloaders, model, criterion, optimizer, scheduler, writer, Defaults.NUM_EPOCHS)
# 
#         if model_wts:
#             model.load_state_dict(model_wts)
#             save_path = save_path + "/" + "model.pt"
#             if not os.path.exists(Defaults.SAVE_PATH):
#                 os.makedirs(Defaults.SAVE_PATH)
